"""
Pally MCP Server - Main server implementation
...
# Log PALLY_MCP_FORCE_ENV_OVERRIDE configuration for transparency
if env_override_enabled():
    logger.info("PALLY_MCP_FORCE_ENV_OVERRIDE enabled - .env file values will override system environment variables")
    logger.debug("Environment override prevents conflicts between different AI tools passing cached API keys")
else:
    logger.debug("PALLY_MCP_FORCE_ENV_OVERRIDE disabled - system environment variables take precedence")


# Create the MCP server instance with a unique name identifier
# This name is used by MCP clients to identify and connect to this specific server
server: Server = Server("pally-server")
...
PROMPT_TEMPLATES = {
    ...
    "version": {
        ...
        "description": "Show server version and system information",
            "template": "Show Pally MCP Server version",
    },
}
...
async def main():
    ...
    # Log startup message
    logger.info("Pally MCP Server starting up...")
    ...
    if IS_AUTO_MODE:
        handshake_instructions = (
            ...
            "When no model is mentioned, first use the `listmodels` tool from Pally to obtain available models to choose the best one from."
        )
    else:
        handshake_instructions = (
            "When the user names a specific model (e.g. 'use chat with gpt5'), send that exact model in the tool call. "
...
def run():
    ...
    try:
        ...
        parser.add_argument(
            "--version",
            action="store_true",
            help="Print Pally MCP Server version and exit.",
        )
